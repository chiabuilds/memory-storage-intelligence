...
RECIPIENT_EMAILS = ["austin.chia@hpe.com"]
...
def translate_foreign_titles(df: pd.DataFrame) -> pd.DataFrame:
    def is_foreign(text):
        return bool(re.search(r'[\uAC00-\uD7A3\u1100-\u11FF\u3130-\u318F\u3040-\u309F\u30A0-\u30FF]', text))
    foreign_mask = df["title"].apply(is_foreign)
    if not foreign_mask.any():
        return df
    foreign_titles = df.loc[foreign_mask, "title"].tolist()
    numbered = "\n".join(f"[{i+1}] {t}" for i, t in enumerate(foreign_titles))
    client = anthropic.Anthropic()
    message = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=1024,
        messages=[{"role": "user", "content": f"Translate these article titles to English. Return only the translations numbered the same way, no explanation.\n\n{numbered}"}],
    )
    ...
    for i, idx in enumerate(df.index[foreign_mask].tolist()):
        if (i + 1) in translations:
            df.at[idx, "title"] = translations[i + 1]
    return df
    ...
    message = client.messages.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                '  "key_takeaway": "2-3 sentence executive summary focused on NAND/DRAM/AI chips, bold 2-3 key terms with **bold**, use [N] citations",\n'
                '  "sections": [\n'
...
def expand_citations(text: str) -> str:
    """Convert [1, 2, 3] style citations to [1][2][3]."""
    def expand(match):
...
def bullet_to_html(text: str, articles: pd.DataFrame) -> str:
    """Render bullet: bold black label before colon, bold blue highlights after colon, clickable citations."""
    def linkify(t):
...
def build_markdown(result: dict, date_str: str, date_range: tuple) -> str:
    ...
    lines = [
        f"# Memory & Storage Market: 3-Day Update — {date_str} (Past 3 Days: {range_label})",
...
def build_html(result: dict, date_str: str, date_range: tuple) -> str:
    ...
    footnotes_html = (
        f'<table style="width:100%;border-collapse:collapse;font-size:13px;font-family:Aptos,Arial,sans-serif;">'
    ...
    <html><body style="font-family:Aptos,Arial,sans-serif;max-width:700px;margin:auto;padding:20px;">
      <h1 style="color:#1a1a2e;font-family:Aptos,Arial,sans-serif;font-size:18px;">Memory & Storage Market: 3-Day Update</h1>
      <p style="color:#888;font-family:Aptos,Arial,sans-serif;font-size:13px;">{date_str} &nbsp;|&nbsp; Past 3 Days: {range_label}</p>
...
def main():
    ...
    df = translate_foreign_titles(df)
    ...
    send_email(f"Memory & Storage Market: 3-Day Update — {date_str}", markdown, html)
